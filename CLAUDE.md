...
./run-public-media-pipeline.sh 04       # only the audio slimming
./run-public-media-pipeline.sh 01 02 03 04   # everything
```
...
companion subtitles, match plausibility) and step 04's audio plan (which
track gets re-encoded, downmixed or kept — dropping the wrong one loses a
dub). Add a case here rather than reasoning about these by hand.
